...
function the_breadcrumb()
{
    ...
    if (!is_home()) {
        ...
        echo "<li>Archiwum autora";
        echo '</li>';
    } elseif (isset($_GET['paged']) && !empty($_GET['paged'])) {
        echo "<li>Archiwum bloga";
        echo '</li>';
    } elseif (is_search()) {
        echo "<li>Wynik wyszukiwania";
        echo '</li>';
    }
    echo '</ol>';
}
...
function shiba_remove_rewrite_rules($permastruct, $ep_mask = EP_NONE)
{
    ...
    // Get first rule
    $rule1 = reset($remove_rules);
    $key_rule1 = key($remove_rules);

    $rules = get_option('rewrite_rules');
    $i = $num_rules;
    foreach ($rules as $pretty_link => $query_link) {
        // find the first rule
        if (($pretty_link == $key_rule1) && ($query_link == $rule1)) {
            $i = 0;
        }
        if ($i < $num_rules) {
            // Delete next $num_rules
            unset($rules[$pretty_link]);
            $i++;
        }
    }
    update_option('rewrite_rules', $rules);
